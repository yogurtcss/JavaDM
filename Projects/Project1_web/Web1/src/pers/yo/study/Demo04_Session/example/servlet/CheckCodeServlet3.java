...
@WebServlet("/checkCodeServlet3") //第一步先把这个注解改了！
public class CheckCodeServlet3 extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        *  */
        ...
        /* 2019-12-22 09:14:42
        * 将生成的验证码X存到 session中，在服务器端通过session拿到此正确的验证码X；
        * 把用户输入的验证码Y 与 服务器端捏着的验证码X 进行比较！！
        * 正确 → ?
        * 错误 → ?
        *  */
        StringBuilder sb = new StringBuilder(); //存储正确的验证码
        ...
        for( int i=1; i<=4; i++ ){
            ...
            *  */
            ...
            sb.append(ch); //把正确的验证码 每一个字符 存进 StringBuilder实例对象中

            g.drawString( String.valueOf(ch), width/5*i, height/2 );
            //转换为字符串最简单的方式：ch+"" 加空字符串
        }

        String checkCode_session = sb.toString(); //把sb转为字符串
        //将验证码存入 请求request的session中
